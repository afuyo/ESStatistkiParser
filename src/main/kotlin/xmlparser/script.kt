...
fun main(args: Array<String>) {
    ...
    val fileName = arguments.getString("file-name")
    ...
    /** Thread that reads xml file and fills data list */
     ...
     /**   val fileStream = Thread.currentThread().contextClassLoader.getResourceAsStream(filePath)
        val zipStream = ZipInputStream(fileStream).apply {
           nextEntry
        }**/
        ...
        //dataList.put(xmlReader.elementText.toString())
            isIdent = true
             ...
             if(event.isCharacters && isIdent )
             {
                 ...
                 File("fileout.txt").appendText(event.asCharacters().data+System.lineSeparator())
